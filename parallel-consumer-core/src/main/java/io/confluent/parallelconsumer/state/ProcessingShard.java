...
public class ProcessingShard<K, V> {
    ...
    ListMultimap<ShardKey, WorkContainer<K, V>> getWorkIfAvailable(int workToGetDelta) {
        ...
        ListMultimap<ShardKey, WorkContainer<K, V>> workTakenMap = LinkedListMultimap.create();

        var iterator = entries.entrySet().iterator();
        int keyBatchSize = 0;
        while (workTakenMap.size() < workToGetDelta && iterator.hasNext()) {
            ...
            if (pm.couldBeTakenAsWork(workContainer)) {
                if (workContainer.isAvailableToTakeAsWork()) {
                    if (!options.getOrdering().equals(KEY_BATCH_EXCLUSIVE) || (keyBatchSize < options.getBatchSize() && getCountWorkInFlight() < options.getBatchSize())) {
                        ...
                        workTakenMap.put(key, workContainer);
                        keyBatchSize++;
                    } else {
                        break;
                    }
        ...
        if (workTakenMap.size() == workToGetDelta) {
            log.trace("Work taken ({}) exceeds max ({})", workTakenMap.size(), workToGetDelta);
        }
